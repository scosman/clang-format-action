...
#
# Checks all C/C++/Protobuf/CUDA/Objective-C files (.h, .H, .hpp, .hh, .h++, .hxx and .c, .C,
# .cpp, .cc, .c++, .cxx, .proto, .cu, .m) in the provided GitHub repository path
# (arg1) for conforming to clang-format. If no path is provided or provided path
# is not a directory, all C/C++/Protobuf/CUDA/Obj-C files are checked. If any files are
...
# to a C/C++/Protobuf/CUDA/Obj-C file.
format_diff() {
	local filepath="$1"
...
# find all C/C++/Protobuf/CUDA/Obj-C files:
#   h, H, hpp, hh, h++, hxx
#   c, C, cpp, cc, c++, cxx
#   ino, pde
#   proto
#   cu
#   m
src_files=$(find "$CHECK_PATH" -name .git -prune -o -regextype posix-egrep -regex '^.*\.((((c|C)(c|pp|xx|\+\+)?$)|((h|H)h?(pp|xx|\+\+)?$))|(ino|pde|proto|cu|m))$' -print)
